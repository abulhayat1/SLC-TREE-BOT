...
module.exports = class SayCommand extends Command {
      ...
      args: [
        {
          key: "Link",
          prompt:
            "** Step :one: : Tree Code or Link.**  `eg:5D4GZW3 or https://forestapp.cc/join-room?token=5D4GZW3` (1/5)",
          type: "string",
          validate: (Link) => {
        ...
        },
        {
          key: "tree_name",
          prompt:
            "** Step :two: : What Is The Tree Name ?**  `eg:Water Spirit` (2/5)",
          type: "string",
          validate: (tree_name) => {
...

        {
          key: "extra_re",
          prompt:
            "** Step :five: : Any Additional Remarks?**  `eg: pom 5/10 or no` (5/5)",
          type: "string",
        },
      ],
    });
  ...
  run(message, { tree_name, Starting_Time, duration, option, Link, extra_re }) {
    ...
    this.client.channels.cache
      .get("660569187105767424")
      .send(`<@&${tree_role_duaration}>` + `<@&${default_tree_Ping}>`, {
        embed: {
          color: "RANDOM",
          title:
            `${m2h(duration)} :deciduous_tree: ${tree_name}\n` +
    ...
    var msg =
      "** :evergreen_tree: Successfully Planted In" +
      " <#" +
